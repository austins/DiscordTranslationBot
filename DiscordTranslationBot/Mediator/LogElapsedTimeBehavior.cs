...
public sealed partial class LogElapsedTimeBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse>
    where TRequest : notnull
{
    private readonly Log _log;

    /// <summary>
    /// Initializes a new instance of the <see cref="LogElapsedTimeBehavior{TRequest,TResponse}" /> class.
    /// </summary>
    /// <param name="logger">Logger to use.</param>
    public LogElapsedTimeBehavior(ILogger<LogElapsedTimeBehavior<TRequest, TResponse>> logger)
    {
        _log = new Log(logger);
    }
    ...
    {
        private readonly ILogger<LogElapsedTimeBehavior<TRequest, TResponse>> _logger;

        public Log(ILogger<LogElapsedTimeBehavior<TRequest, TResponse>> logger)
        {
            _logger = logger;
        }
